...
pub struct AdaptEditor {
    actions: List<Action>
}

impl AdaptEditor {
  pub fn new(initial_actions: List<Action>) -> AdaptEditor {
    AdaptEditor{
...
impl EditorPipeline for AdaptEditor {
    fn take_action(self: &mut Self, ac: Action) -> () {
      self.actions = self.actions.append(ac);
      //self.actions = self.actions.append(Action::Cmd(Command::Ins("A".to_string(),Dir::R)));
    }
